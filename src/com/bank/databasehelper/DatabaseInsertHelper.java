...
public class DatabaseInsertHelper extends DatabaseInserter {
  ...
  public static int insertUserAccount(int userId, int accountId) throws SQLException, DatabaseInsertException {
    ...
    if (cdt1 && cdt2) {
      if (DatabaseSelectHelper.getAccountIds(userId).contains(accountId) == false) {
        Connection connection = DatabaseDriverHelper.connectOrCreateDataBase();
   ...
   * @param userId the id of the user which the message will be associated with.
  ...
  public static int insertMessage(int userId, String message) 
    ...
    boolean cdt1 = Checker.checkValidMessage(message);
    if (cdt1) {
      Connection connection = DatabaseDriverHelper.connectOrCreateDataBase();
      int result = DatabaseInserter.insertMessage(userId, message, connection);
      connection.close();
      return result;
    } else {
      return -1;
